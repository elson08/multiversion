...
import (
	...
	legacypacket "github.com/potlounge/multiversion/protocols/v486/packet"
...
func New() *Protocol {
	...
	return &Protocol{itemMapping: itemMapping, blockMapping: blockMapping,
		itemTranslator:  translator.NewItemTranslator(itemMapping, latest.NewItemMapping(), blockMapping, latest.NewBlockMapping()),
		blockTranslator: translator.NewBlockTranslator(blockMapping, latest.NewBlockMapping(), chunk.NewNetworkPersistentEncoding(blockMapping, BlockVersion), chunk.NewBlockPaletteEncoding(blockMapping, BlockVersion), false)}
}
...
func (Protocol) Packets(_ bool) packet.Pool {
	...
	pool[packet.IDSubChunk] = func() packet.Packet { return &legacypacket.SubChunk{} }
...
func (p Protocol) ConvertToLatest(pk packet.Packet, conn *minecraft.Conn) []packet.Packet {
	...
	//	packet.IDSetScore,
	//}
	//if !slices.Contains(ignoreIds, pk.ID()) {
	//	fmt.Printf("1.18.12 -> Latest: %T\n", pk)
	//}
	...
	case *legacypacket.SubChunk:
		newPks = append(newPks, &packet.SubChunk{
			CacheEnabled: pk.CacheEnabled,
			Dimension:    pk.Dimension,
			Position:     pk.Position,
			SubChunkEntries: lo.Map(pk.SubChunkEntries, func(entry types.SubChunkEntry, _ int) protocol.SubChunkEntry {
	...
	case *packet.BlockActorData:
		newPks = append(newPks, &packet.BlockActorData{
			Position: pk.Position,
			NBTData:  upgradeBlockActorData(pk.NBTData),
		})
	...
	case *legacypacket.SetTitle:
		newPks = append(newPks, &packet.SetTitle{
			ActionType:      pk.ActionType,
			Text:            pk.Text,
	...
	for i, pk := range result {
		...
		case *packet.SubChunk:
			result[i] = &legacypacket.SubChunk{
				...
				Dimension:    pk.Dimension,
				Position:     pk.Position,
				SubChunkEntries: lo.Map(pk.SubChunkEntries, func(entry protocol.SubChunkEntry, _ int) types.SubChunkEntry {
					return types.SubChunkEntry{
						Offset:        entry.Offset,
						Result:        entry.Result,
						RawPayload:    entry.RawPayload,
						HeightMapType: entry.HeightMapType,
		...
		case *packet.BlockActorData:
			result[i] = &packet.BlockActorData{
				Position: pk.Position,
				NBTData:  downgradeBlockActorData(pk.NBTData),
			}
		case *packet.AddPlayer:
			result[i] = &legacypacket.AddPlayer{
				...
				AdventureSettings: packet.AdventureSettings{
					CommandPermissionLevel: uint32(2),
					PermissionLevel:        uint32(2),
		...
		case *packet.AvailableCommands:
			...
			var enums []types.CommandEnum
			for _, enum := range pk.Enums {
				enums = append(enums, types.CommandEnum{
					Type: enum.Type,
					ValueIndices: lo.Map(enum.ValueIndices, func(item uint32, index int) uint {
						return uint(item)
					}),
				})
			}
		...
		case *packet.StartGame:
			...
			result[i] = &legacypacket.StartGame{
				...
				Trial:                          pk.Trial,
				PlayerMovementSettings: types.PlayerMovementSettings{
