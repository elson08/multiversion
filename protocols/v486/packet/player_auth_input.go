...
func (pk *PlayerAuthInput) Marshal(io protocol.IO) {
	...
	if pk.InputData&(1<<packet.InputFlagPerformItemInteraction) != 0 {
		io.PlayerInventoryAction(&pk.ItemInteractionData)
	}

	if pk.InputData&(1<<packet.InputFlagPerformItemStackRequest) != 0 {
		protocol.Single(io, &pk.ItemStackRequest)
	}

	if pk.InputData&(1<<packet.InputFlagPerformBlockActions) != 0 {
